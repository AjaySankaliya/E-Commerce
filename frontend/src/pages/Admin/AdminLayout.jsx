...
import { LayoutDashboard, Users, ShoppingBag, ArrowLeft, Menu, X, Bell } from 'lucide-react';
...
const AdminLayout = () => {
  ...
  return (
    <div className="h-screen bg-slate-50 flex font-sans overflow-hidden">
      ...
      <aside className={`fixed inset-y-0 left-0 w-72 bg-slate-900 text-white flex flex-col z-50 transition-transform duration-500 ease-in-out transform ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 md:static md:z-auto h-screen shadow-2xl overflow-y-auto`}>
        <div className="p-8 border-b border-slate-800/50 flex items-center justify-between flex-shrink-0">
        ...
        <div className="px-5 py-10 flex-1 space-y-2 overflow-y-auto">
          ...
          <NavLink to="/admin/products" className={({ isActive }) => `flex items-center gap-4 px-6 py-4 rounded-2xl font-bold transition-all duration-300 ${isActive ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/20' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}>
            <ShoppingBag size={20} /> Manage Products
          </NavLink>
        </div>

        <div className="p-6 border-t border-slate-800/50 space-y-4 flex-shrink-0">
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden">
        {/* Top Navbar */}
        <header className="h-20 bg-white/80 backdrop-blur-md border-b border-slate-200/60 px-6 sm:px-10 flex items-center justify-between flex-shrink-0 z-20 transition-all">
          ...
          <div className="flex items-center gap-4 sm:gap-6">
            ...
            <div className="flex items-center gap-3 border-l pl-6 border-slate-200">
              ...
              <div className="w-10 h-10 rounded-full bg-gradient-to-tr from-blue-600 to-indigo-600 flex items-center justify-center text-white font-black border-2 border-white shadow-lg flex-shrink-0">
